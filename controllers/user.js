...
exports.createOrder = async (req, res) => {
  ...
  let bluckOption = products.map((item) => {
    return {
      updateOne: {
  ...
  let updated = await Product.bulkWrite(bluckOption, {});
...
exports.createCashOrder = async (req, res) => {
  ...
  if (!COD) return res.josn(400).send("Create cash order failed");
  ...
  let bluckOption = userCart.products.map((item) => {
    return {
      updateOne: {
        filter: { _id: item.product._id },
        update: { $inc: { quantity: -item.count, sold: +item.count } },
